...
#define RMI_HASH_ALGO_SHA256		(0U)
#define RMI_HASH_ALGO_SHA512		(1U)
...
#define RMI_NO_MEASURE_CONTENT		(0U)
#define RMI_MEASURE_CONTENT			(1U)
#define RPV_SIZE					64
...
#define L1_GRANULE                  0x40000000  /* 1GB */
...
typedef struct rmi_cvm_create_params {
    uint64_t flags;
    uint64_t s2sz;
    uint64_t sve_vl;
    uint64_t num_bps;
...
} rmi_cvm_create_params_t;

typedef struct rmi_tec_crreate_params {
    uint64_t gprs[REC_CREATE_NR_GPRS];
    uint64_t pc;
    uint64_t flags;
} rmi_tec_create_params_t;

typedef struct rmi_data_crreate_params {
    uint64_t *data;
    uint64_t size;
    uint64_t flags;
    uint64_t ipa;
} rmi_data_create_params_t;

/* TmmMeasurementDescriptorCVM type as per RMM spec */
typedef struct rmi_measure_cvm {
    /* Flags */
...
typedef struct rmi_measure_data {
...
typedef struct rmi_measure_tec {
...
typedef struct rmi_measure_ripas {
