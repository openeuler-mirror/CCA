    ...
    while (ret == QCBOR_SUCCESS) {
        ...
        if (item.uLabelType == QCBOR_TYPE_INT64) {
            ...
            switch (item.label.int64) {
                ...
                case CCA_CVM_PROFILE:
                    if (!check_item_tstring_size(&item, NULL, 0)) {
                        printf("Realm token profile is not in expected format\n");
                        return VIRTCCA_ERROR;
                    }
                    claim->profile = item.val.string;
                    break;
                ...
                case CCA_CVM_PUB_KEY:
                    ...
                    */
                    if (!check_item_bstring_size(&item, (int[]){CCA_BYTE_SIZE_133, CCA_BYTE_SIZE_550, CCA_BYTE_SIZE_107, CCA_BYTE_SIZE_143}, 4)) {
                        printf("cVM public key is not in expected format, len=%u\n", item.val.string.len);
                        return VIRTCCA_ERROR;
                    }
...
#ifdef CCA
    printf("\tProfile: ");
    _p = token->cvm_token.profile.ptr;
    _l = token->cvm_token.profile.len;
    for (unsigned int i = 0U; i < _l; ++i) {
        printf("%c", _p[i]);
    }
    printf("\n");
