...
#define REPORT_PATH "/sys/kernel/config/tsm/report/report0"
#define INBLOB_PATH  REPORT_PATH "/inblob"
#define OUTBLOB_PATH REPORT_PATH "/outblob"
#define BLOB_SIZE 64

bool use_fde = false;

static int cca_get_dev_cert(unsigned char *dev_cert, size_t *dev_cert_len)
{
    memcpy(dev_cert, "test", strlen("test"));

    return 0;
}

static int cca_get_attestation_token(unsigned char *challenge, size_t challenge_len, unsigned char *token, size_t *token_len)
{
    if (challenge == NULL || challenge_len != BLOB_SIZE || token == NULL || token_len == NULL)
        return -1;
    
    // input the challenge.
    FILE *fp = fopen(INBLOB_PATH, "wb");
    if (!fp) {
        fprintf(stderr, "open %s failed : %s\n", INBLOB_PATH, strerror(errno));
        return -1;
    }
    if (fwrite(challenge, 1, BLOB_SIZE, fp) != BLOB_SIZE) {
    ...
    if (fd == -1) {
        fprintf(stderr, "open %s failed.: %s\n", OUTBLOB_PATH, strerror(errno));
        return 1;
    }

    size_t read_bytes = read(fd, token, *token_len);
    if (read_bytes < 0) {
        fprintf(stderr, "read %s failed: %s\n", OUTBLOB_PATH, strerror(errno));
    }
    close(fd);
    *token_len = read_bytes;

    return 0;
}

int handle_connect(int connfd)
{
    int ret;
