...
{
	/*
	 * The current test platform does not support the capability register,
	 * therefore, the version is used for check.
	 */
	if (qm->ver < QM_HW_V3)
		return false;

	if (qm->cap_ver && !test_bit(QM_SUPPORT_DAE, &qm->caps))
		return false;
