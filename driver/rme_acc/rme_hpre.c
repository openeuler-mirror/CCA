...
static const char rme_hpre_name[] = "rme_hpre";
...
static const struct hisi_qm_cap_query_info hpre_cap_query_info[] = {
	{HPRE_QM_RAS_NFE_TYPE,  "HPRE_QM_RAS_NFE_TYPE  ", 0x3124, 0x0, 0x1C37, 0x7C37},
	{HPRE_QM_RAS_NFE_RESET, "HPRE_QM_RAS_NFE_RESET ", 0x3128, 0x0, 0xC77, 0x6C77},
	{HPRE_QM_RAS_CE_TYPE,   "HPRE_QM_RAS_CE_TYPE   ", 0x312C, 0x0, 0x8, 0x8},
...
static struct hisi_qm_list rme_hpre_devices;
struct hisi_qm_list *get_rme_hpre_devices(void)
{
	return &rme_hpre_devices;
}
...
static int hpre_set_cluster(struct hisi_qm *qm)
{
	...
	clusters_num = (hpre_core_info >>
			hpre_basic_info[HPRE_CLUSTER_NUM_CAP].shift) &
	...
	for (i = 0; i < clusters_num; i++) {
		...
		if (ret) {
			dev_err(dev, "Cluster %u int status timeout!\n", i);
			return -ETIMEDOUT;
		}
	}
...
static int hpre_set_user_domain_and_cache(struct hisi_qm *qm)
{
	...
	writel(HPRE_RSA_ENB | HPRE_ECC_ENB, qm->io_base + HPRE_TYPES_ENB);
...
static int hpre_pf_comm_regs_debugfs_init(struct hisi_qm *qm)
{
	...
	debugfs_create_file("regs", 0444, qm->debug.debug_root, regset,
			    &hpre_com_regs_fops);
...
static void hpre_dfx_debug_init(struct hisi_qm *qm)
{
	...
	if (hpre_regs)
		debugfs_create_file("diff_regs", 0444, parent, qm,
				    &hpre_diff_regs_fops);
...
static int hpre_pre_store_cap_reg(struct hisi_qm *qm)
{
	...
	for (i = 0; i < size; i++) {
		...
		hpre_cap[i].cap_val = hisi_qm_get_cap_value(qm,
							    hpre_cap_query_info,
							    i, qm->cap_ver);
	}
	...
	if (clusters_num > HPRE_CLUSTERS_NUM_MAX) {
		dev_err(dev, "Device cluster num %u is out of max range %d!\n",
...
static int hpre_show_last_regs_init(struct hisi_qm *qm)
{
	...
	debug->last_words = kcalloc(cluster_dfx_regs_num * clusters_num +
				    com_dfx_regs_num, sizeof(unsigned int),
				    GFP_KERNEL);
	if (!debug->last_words)
		return -ENOMEM;
	...
	for (i = 0; i < com_dfx_regs_num; i++) {
		...
		if (debug->last_words[i] != val)
			pci_info(pdev, "Common_core:%s \t= 0x%08x => 0x%08x\n",
				 hpre_com_dfx_regs[i].name,
				 debug->last_words[i], val);
	}
		...
		for (j = 0; j <  cluster_dfx_regs_num; j++) {
			...
			if (debug->last_words[idx] != val)
				pci_info(pdev, "cluster-%d:%s \t= 0x%08x => 0x%08x\n",
					 i, hpre_cluster_dfx_regs[j].name,
...
static void hpre_err_info_init(struct hisi_qm *qm)
{
	...
	qm_err->ce = hisi_qm_get_hw_info(qm, hpre_basic_info,
					 HPRE_QM_CE_MASK_CAP, qm->cap_ver);
	qm_err->nfe = hisi_qm_get_hw_info(qm, hpre_basic_info,
					  HPRE_QM_NFE_MASK_CAP, qm->cap_ver);
	...
	dev_err->ce = hisi_qm_get_hw_info(qm, hpre_basic_info, HPRE_CE_MASK_CAP,
					  qm->cap_ver);
	dev_err->nfe = hisi_qm_get_hw_info(qm, hpre_basic_info,
					   HPRE_NFE_MASK_CAP, qm->cap_ver);
...
int hpre_probe(struct pci_dev *pdev)
{
	...
	if (pdev->vendor != PCI_VENDOR_ID_HUAWEI ||
	    pdev->device != PCI_DEVICE_ID_HUAWEI_HPRE_PF)
		return -EINVAL;
	...
	hisi_qm_add_list(qm, &rme_hpre_devices);
...
void hpre_remove(struct pci_dev *pdev)
{
	...
	hisi_qm_del_list(qm, &rme_hpre_devices);
